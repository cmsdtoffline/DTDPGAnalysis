          ...
          UnpackingRpcRecHitToken_(consumes<RPCRecHitCollection> (pset.getParameter<edm::InputTag>("UnpackingRpcRecHitLabel")))
{
    ...
    hoTPLabel = pset.getParameter<edm::InputTag>("hoTPLabel");
...
void TTreeGenerator::analyze(const edm::Event& event, const edm::EventSetup& context)
{
    ...
    event.getByToken(tok_hoTP, hotpHandle);
...
void TTreeGenerator::fill_hoTP_variables(edm::Handle <edm::SortedCollection <HOTriggerPrimitiveDigi> > hotpHandle)
{
            ...
            hoTP != hotpHandle->end(); ++hoTP)
       {
            ...
            /*printf("hoTPdigi: bits %d, whSamp %d, data_whSamp %d, mip %d \n", \
                hoTP->bits(), \
...
void TTreeGenerator::beginJob()
{
    ...
    //HO-TP variables
    tree_->Branch("hoTPdigi_bits", &hoTPdigi_bits);
    tree_->Branch("hoTPdigi_iEta", &hoTPdigi_iEta);
    tree_->Branch("hoTPdigi_iPhi", &hoTPdigi_iPhi);
    tree_->Branch("hoTPdigi_nSamples", &hoTPdigi_nSamples);
    tree_->Branch("hoTPdigi_raw", &hoTPdigi_raw);
    tree_->Branch("hoTPdigi_whichSampleTriggered", &hoTPdigi_whichSampleTriggered);
